...
#### LOAD DATA
####
airports = File.read('/Users/wyattkim/Desktop/trails.dat').split("\n")
records = {}
airports.each do |airport|
  data = airport.split(',').map { |value| value.delete('"') }
  id, name, forest, status, lng, lat, description = data
  id = id.to_i
  lat = lat.to_f
  lng = lng.to_f
...
index.set_settings(attributesToIndex: %w(objectID name status _geoloc description),
